...
def repair(game, original_actions, actions, broken_action, data, pry_db: true)
  ...
  # copied migrate_game.rb, updating only this section
  #
  # 1822 family - discard trains earlier
  if game.active_step.is_a?(Engine::Game::G1822::Step::DiscardTrain) ||
      game.active_step.is_a?(Engine::Game::G1822CA::Step::DiscardTrain)
    discard_train_action = next_actions.find do |action|
      action['type'] == 'discard_train' && action['entity'] == current_entity.id
    end

    unless discard_train_action
      require 'pry-byebug'
      binding.pry

      raise Exception, 'Could not find discard_train_action'
    end

    actions.delete(discard_train_action)
    actions.insert(broken_action_idx, discard_train_action)
    return
  end
