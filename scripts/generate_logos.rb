...
def generate_logos(game_title, simple=false)
  ...
  corporations = game.init_corporations(game.stock_market)
  ...
  corporations.each do |corp|
    ...
    filename =
      if simple
        "public/#{corp.simple_logo}.alt.svg"
      else
        "public/#{corp.logo}"
      end
    ...
    if File.exist?(filename)
      ...
      svg = <<-SVG
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 8 8"><circle cx="4" cy="4" r="4" fill="#{corp.color}"/><text x="4" y="5" text-anchor="middle" font-weight="700" font-size="3" font-family="Lato" fill="#{corp.text_color}">#{corp.id}</text></svg>
      SVG
