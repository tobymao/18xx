...
module Engine
  describe Game::G18ZOO::Game do
    shared_examples "a purchasable power" do
      ...
      it "should be inside available_companies" do
      ...
      it "can be purchased" do
        ...
        expect(player_1.cash).to eq(starting_money - price)
      end
    end
    ...
    describe 'Power' do
      ...
      describe 'Midas' do
        ...
        context 'when player buys it' do
          ...
          it 'should add a choice for player' do
            expect(game.round.active_step.actions(game.current_entity)).to include 'choose'
            expect(game.round.active_step.choices).to include :midas
          end
      ...
      describe 'Whatsup' do
        ...
        # describe 'Whatsup' do
        #   let(:game) { Engine::Game::G18ZOO::Game.new(players, id: 'hs_gpptfzdv_1612649034') }
        #   let(:current_power) { game.available_companies.find { |c| c.sym.to_sym == :WHATSUP } }
        #   let(:player_1) { game.players.first }
        #   let(:corporation) { game.corporations.first }
        #   let(:share_price) { game.stock_market.par_prices.find { |par_price| par_price.price == 5 } }
        #
        ...
        #   end
        #
        #   it 'should have a choice after par only if enough money' do
        #     expect(game.round.active_step.choices).to be_empty
        #
        #     game.round.process_action(Action::Par.new(player_1, corporation: corporation, share_price: share_price))
        #     game.round.process_action(Action::Pass.new(corporation))
        #     expect(game.round.active_step.choices).to include "whatsup:#{corporation.name}:2S-0"
        #
        #     3.times { |_| game.round.process_action(Engine::Action::Pass.new(game.current_entity)) }
        #
        #     next_or!
        #     game.phase.buying_train!(corporation, game.trains.find { |t| t.name == '3S' })
        #     next_sr!
        #
        #     p "d"
        #     expect(game.round.active_step.choices).to be_empty
        ...
        #     game.round.process_action(Action::BuyShares.new(player_1, shares: corporation.shares[0]))
        #     p "f"
        #     expect(game.round.active_step.choices).to include "whatsup:#{corporation.name}"
        #   end
        # end
      end
      ...
      describe 'Leprechaun pot of gold' do
        ...
        context 'when player buys it on Monday SR' do
          before do
            next_sr!
