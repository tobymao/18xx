  ...
  describe Round::Stock do
    ...
    describe '#can_buy?' do
      ...
      it "can't buy when at cert limit when doing so would gain you the presidency" do
        ...
        3.times { game.share_pool.buy_share(player_1, corp_0.shares[0]) }

        # Get player 0 to the same quantity and to cert limit
        4.times { game.share_pool.buy_share(player_0, corp_0.shares[0]) }
        4.times { game.share_pool.buy_share(player_0, corp_1.shares[0]) }
        3.times { game.share_pool.buy_share(player_0, corp_2.shares[0]) } # at 6-player cert limit
