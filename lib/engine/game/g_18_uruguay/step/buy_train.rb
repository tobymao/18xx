      ...
      module Step
        class BuyTrain < Engine::Step::BuyTrain
          ...
          def buyable_trains(entity)
            trains = super
            trains = trains.select!(&:from_depot?) if entity == @game.rptla
            trains = trains.filter { |train| ship_variant?(train) } if entity == @game.rptla
            trains = trains.reject { |train| train.name == '7' } unless entity == @game.fce
