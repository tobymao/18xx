    ...
    module G18TS
      class Game < Game::Base
        ...
        DUMMY_TRAINS = [
          ...
          { name: 'P3', price: 1, distance: [{ 'nodes' => %w[city town], 'pay' => 3, 'visit' => 3 }] },
          {
            name: '3+',
            price: 1,
            distance: [{ 'nodes' => ['city'], 'pay' => 3, 'visit' => 3 },
          ...
          { name: '2D', price: 1, distance: [{ 'nodes' => %w[city town], 'pay' => 2, 'visit' => 2 }] },
          { name: '7', price: 1, distance: [{ 'nodes' => %w[city town], 'pay' => 7, 'visit' => 7 }] },
          { name: '3D', price: 1, distance: [{ 'nodes' => %w[city town], 'pay' => 3, 'visit' => 3 }] },
        ].freeze
        ...
        def game_trains
          ...
          when '1'
            add_train_list(trains, { '2' => 7, '3' => 6, '4' => 6, '5' => 6,'P3' => 1,'3+' => 1,'6' => 5, '2D' => 1,'7' => 10})
          when '2'
            add_train_list(trains, { '2' => 5, '3' => 5, '4' => 5, '5' => 6, 'P3' => 2,'6' => 3, '7' => 10  })
          when '3' # all units          
            add_train_list(trains, { '2' => 7, '3' => 6, '4' => 8, '5' => 7, 'P3' => 5, '3+' => 4 ,'6' => 3, '2D' => 3, '7' => 10, '3D' => 2})
          end
        ...
        TILE_200 = '200'
        TILE200_HEXES = %w[O17 R12 T14].freeze
        BANK_UNIT1 = 5000
        BANK_UNIT2 = 5000
        BANK_UNIT3 = 4000
        EVENTS_TEXT = Base::EVENTS_TEXT.merge('signal_end_game' => ['Trigger End Game',
                                                          '']).freeze
        def init_optional_rules(optional_rules)
          ...
          if optional_rules.empty?
            ...
            when 3, 4
              ...
              @log << 'Using West Scenario based on player count.'
            when 5
              optional_rules << :unit_1
              @log << 'Using East Scenario based on player count.'
            when 6, 7, 8, 9
              optional_rules << :unit_3
              @log << 'Using Full Scenario based on player count.'
            end
          end
        ...
        def cash_by_options
          case @units.keys.sort.map(&:to_s).join
          when '1'
            { 2 => 1260, 3 => 840, 4 => 630, 5 => 504, 6 => 420, 7 => 360 }
          when '2'
            { 2 => 1260, 3 => 840, 4 => 630, 5 => 504 }
          when '3'
        ...
        def certs_by_options
          certs = case @units.keys.sort.map(&:to_s).join
                  when '1'
                    { 3 => 36, 4 => 27, 5 => 22, 6 => 17, 7 => 16}
                  when '2'
                    { 3 => 31, 4 => 24, 5 => 19 }
                  when '3'
                    { 4 => 37, 5 => 31, 6 => 26, 7 => 21, 8 => 19, 9 => 17 }
                  end
        ...
        def routes_revenue(routes)
          revenue = super
          revenue += 400 if @round.current_operator.share_price.type=='endgame' unless revenue.zero?
          revenue
        end
        ...
        def event_signal_end_game!
          if @units[3]
            final_operating_round_count=4
          else
            final_operating_round_count=3
          end
          
          @final_operating_rounds = final_operating_round_count-@operating_rounds+@round.round_num
          @log << "First 7 train purchased, ending game at conclusion of OR #{@turn + 1}.#{@final_operating_rounds}"
        end
      end
    end
  end
