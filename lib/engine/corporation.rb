...
module Engine
  class Corporation
    ...
    def holding_ok?(share_holder, extra_percent = 0)
      preferred_percent = share_holder.preferred_percent_of(self) + extra_percent
      %i[multiple_buy unlimited].include?(@share_price&.type) || preferred_percent <= @max_ownership_percent
    end
