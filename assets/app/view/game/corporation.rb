    ...
    class Corporation < Snabberb::Component
      ...
      def entities_rows(entities)
        ...
        entity_info = entities.map do |entity|
          [
            ...
            @game.respond_to?(:share_flags) && @game&.share_flags(entity.shares_of(@corporation)),
          ]
        end
      ...
      def render_shares
        ...
        if player_rows.any? || @corporation.num_market_shares.positive?
          ...
          other_flags = @game.respond_to?(:share_flags) && @game.share_flags(@game.share_pool.shares_of(@corporation))
