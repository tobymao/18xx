      ...
      class City < Base
        ...
        def preferred_render_locations
          if @edge
            ...
            weights += EXTRA_SLOT_REGIONS[@edge] unless @city.total_slots == 1
            distance = 50
            # move in if city is on a "half" edge and has more that one slot
            distance -= 8 if @edge.to_i != @edge && @city.total_slots > 1
        ...
        def render_part
          slots = (0..@city.total_slots - 1).zip(@city.tokens + @city.bull_tokens).map do |slot_index, token|
            ...
            # rotation
            x, y = CITY_SLOT_POSITION[@city.total_slots]
        ...
        def render_revenue
          ...
          displacement = REVENUE_DISPLACEMENT[layout][@city.total_slots]
          ...
          if @num_cts == 1
            ...
            regions = if layout == :flat
                        @city.total_slots == 1 ? { [9, 16] => 1.0, [10, 17] => 0.5, [11, 18] => 0.25 } : [11, 18]
                      else
                        @city.total_slots == 1 ? { [8, 9] => 1.0, [2, 10] => 0.5, [3, 4] => 0.25 } : [3, 4]
                      end
          elsif @edge && @city.total_slots == 1
